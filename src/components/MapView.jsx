...
export default function MapView() {
  ...
  useEffect(() => {
    const initMap = async () => {
      ...
      // Phoenix villages colored by homelessness severity (derived)
      ...
      const showPhoenixHomelessnessSyntheticPopup = (e) => {
        ...
        new mapLib.current.Popup({ closeButton: true, maxWidth: '320px', className: 'popup-health' })
          .setLngLat(coords)
          .setHTML(`
            <div style="font-size:13px;line-height:1.45;color:rgba(255,255,255,0.9);min-width:240px">
              <div style="font-size:11px;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;color:rgba(255,255,255,0.4);margin-bottom:6px">
                Phoenix · Homelessness services
              </div>
              <div style="font-weight:600;font-size:14px;color:#fff;margin-bottom:6px;line-height:1.3">
                ${p.category || 'Service category'}
              </div>
              <div style="color:rgba(255,255,255,0.6);font-size:12px;margin-bottom:10px">
                Period: ${snapLabel || p.periodLabel || '—'}
              </div>
              ${p.neighborhood ? `
                <div style="color:rgba(255,255,255,0.55);font-size:12px;margin-bottom:10px">
                  Neighborhood: <span style="color:rgba(255,255,255,0.85);font-weight:600">${p.neighborhood}</span>
                </div>
              ` : ''}
      ...
      // Phoenix affected neighborhoods: hover, select, tooltip
      ...
      map.current.on('mousemove', 'phoenix-villages-homelessness-fill', (e) => {
        const f = e.features?.[0]
        const id = f?.properties?.NAME
        if (!id) return
        if (hoveredVillageId && hoveredVillageId !== id) {
          map.current.setFeatureState({ source: 'phoenix-villages-homelessness', id: hoveredVillageId }, { hover: false })
        }
        hoveredVillageId = id
        map.current.setFeatureState({ source: 'phoenix-villages-homelessness', id }, { hover: true })
      })

      map.current.on('mouseleave', 'phoenix-villages-homelessness-fill', () => {
        if (hoveredVillageId) {
          map.current.setFeatureState({ source: 'phoenix-villages-homelessness', id: hoveredVillageId }, { hover: false })
      ...
      map.current.on('click', 'phoenix-villages-homelessness-fill', (e) => {
        const f = e.features?.[0]
        if (!f) return
        const id = f?.properties?.NAME
        if (!id) return
        ...
        if (selectedVillageId && selectedVillageId !== id) {
          map.current.setFeatureState({ source: 'phoenix-villages-homelessness', id: selectedVillageId }, { selected: false })
        }
        const nextSelected = selectedVillageId !== id
        selectedVillageId = nextSelected ? id : null
        map.current.setFeatureState({ source: 'phoenix-villages-homelessness', id }, { selected: nextSelected })
          ...
          ? servedRows
              .map((r) => `
                <div style="display:flex;justify-content:space-between;gap:12px;margin-top:4px">
                  <span style="color:rgba(255,255,255,0.75)">${r.category}</span>
                  <span style="font-weight:700;color:${spec.nameColor}">${Number(r.value || 0).toLocaleString()}</span>
                </div>
              `)
              .join('')
          : ''

        homelessnessVillagePopup = new mapLib.current.Popup({ closeButton: false, closeOnClick: false, maxWidth: '260px', className: 'popup-homelessness-card' })
          .setLngLat(center)
          .setHTML(`
            <div style="
              min-width:200px;
            ...
            ">
              ...
              ">
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;row-gap:4px;flex:1 1 auto;min-width:0">
                  <span style="font-size:10px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:${spec.labelColor}">Homelessness services</span>
                </div>
              </div>

              <div style="font-weight:600;font-size:16px;color:${spec.nameColor};margin-bottom:6px;line-height:1.3">${id}</div>

              <div style="color:${spec.bodySubtext};font-size:12px;margin-bottom:2px">
                ${Number.isFinite(servedTotal) ? servedTotal.toLocaleString() : '—'} people served <span style="opacity:0.9">${topSummary}</span>
              </div>

              ${rows ? `<div style="margin-top:8px;border-top:1px solid rgba(255,255,255,0.08);padding-top:8px">${rows}</div>` : ''}
            </div>
          `)
          .addTo(map.current)
      })
  ...
  useEffect(() => {
    ...
    const shouldShow = selectedCity === 'phoenix' && phoenixCouncilDistrictBoundariesVisible
    const visibility = shouldShow ? 'visible' : 'none'

    const applyVisibility = () => {
      if (!map.current) return
      if (map.current.getLayer('phoenix-council-districts-fill')) map.current.setLayoutProperty('phoenix-council-districts-fill', 'visibility', visibility)
      if (map.current.getLayer('phoenix-council-districts-border')) map.current.setLayoutProperty('phoenix-council-districts-border', 'visibility', visibility)
      if (map.current.getLayer('phoenix-council-districts-labels')) map.current.setLayoutProperty('phoenix-council-districts-labels', 'visibility', visibility)
    }
  ...
  }, [selectedCity, phoenixCouncilDistrictBoundariesVisible, phoenixCouncilDistrictsGeojson, phoenixCouncilDistrictsEnabled, mapLoaded])
  ...
  useEffect(() => {
    if (!map.current || !mapLoaded) return
    if (!map.current.getSource('phoenix-villages-homelessness')) return
    ...
    setVis('phoenix-villages-homelessness-fill', shouldShow ? 'visible' : 'none')
    setVis('phoenix-villages-homelessness-border', shouldShow ? 'visible' : 'none')

    if (!shouldShow) {
      map.current.getSource('phoenix-villages-homelessness').setData({ type: 'FeatureCollection', features: [] })
      return
    }

    // Count homelessness points per village using the currently-enabled homelessness categories.
    ...
    const counts = new Map()
    const byCatCounts = new Map() // village -> Map(category -> count)
    for (const f of visiblePts) {
      const nb = f?.properties?.neighborhood
      if (!nb) continue
      counts.set(nb, (counts.get(nb) || 0) + 1)

      const cat = f?.properties?.category || 'Unknown'
      const inner = byCatCounts.get(nb) || new Map()
      inner.set(cat, (inner.get(cat) || 0) + 1)
      byCatCounts.set(nb, inner)
    }
    ...
    const affectedFeatures = baseVillages.features
      .map((f) => {
        const name = f?.properties?.NAME
        const c = name ? (counts.get(name) || 0) : 0
        if (c <= 0) return null
        const inner = name ? (byCatCounts.get(name) || new Map()) : new Map()
    ...
    map.current.getSource('phoenix-villages-homelessness').setData({
      type: 'FeatureCollection',
